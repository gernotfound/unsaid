...
function message(code: string) {
  ...
  if (code === "ORDER_NOT_READY_FOR_SHIPMENT") return "L'ordine deve essere in processing prima della spedizione.";
  ...
  if (code === "SHIPMENT_NOT_READY_FOR_DELIVERY") return "La spedizione deve essere prima marcata come shipped.";
  ...
  async function action(actionName: "shipment_ready" | "shipped" | "delivered") {
    ...
    try {
      ...
      setNotice(actionName === "shipment_ready"
        ...
        : actionName === "shipped"
          ? "Ordine marcato shipped e notifica spedizione accodata."
          : "Ordine marcato delivered.");
    } catch (error) {
      setNotice(message(error instanceof Error ? error.message : String(error)));
    } finally {
      setBusy(false);
    ...
    return (
      <section className={styles.center}>
        <form className={styles.login} onSubmit={login}>
          <p className={styles.kicker}>UNSAID / FULFILLMENT</p>
          <h1>Admin.</h1>
  ...
  return (
    <section className={styles.shell}>
      <header className={styles.topbar}>
        <div><p className={styles.kicker}>UNSAID / FULFILLMENT</p><strong>Shipments + customer notifications</strong></div>
      ...
      <div className={styles.workspace}>
        <aside className={styles.sidebar}>
          <div className={styles.summary}><span>Operational orders</span><strong>{operational.length}</strong></div>
        ...
        <main className={styles.editor}>
          {detail ? (
            <>
              ...
              {detail.reviewReason ? <div className={styles.warning}>MANUAL REVIEW / non spedire questo ordine.</div> : null}

              <div className={styles.cards}>
                <section><p className={styles.kicker}>DESTINATION</p><strong>{detail.order.shippingAddress.recipientName}</strong><span>{detail.order.shippingAddress.line1}</span><span>{detail.order.shippingAddress.postalCode} {detail.order.shippingAddress.city} ({detail.order.shippingAddress.province})</span></section>
                <section><p className={styles.kicker}>SHIPMENT</p><strong>{detail.shipment?.status ?? "not prepared"}</strong><span>{detail.shipment?.provider ?? "—"}</span><span>{detail.shipment?.trackingCode ?? "—"}</span>{detail.shipment?.trackingUrl ? <a href={detail.shipment.trackingUrl} target="_blank" rel="noreferrer">Apri tracking</a> : null}</section>
              </div>

              {detail.order.status === "processing" ? (
                <section className={styles.formSection}>
                  <div><p className={styles.kicker}>DISPATCH</p><h2>Prepara spedizione</h2></div>
                  <div className={styles.formGrid}>
                    ...
                    <label><span>Tracking code</span><input value={trackingCode} maxLength={120} onChange={(event) => setTrackingCode(event.target.value)} /></label>
                    <label><span>Tracking URL HTTPS</span><input type="url" value={trackingUrl} maxLength={500} onChange={(event) => setTrackingUrl(event.target.value)} /></label>
                  </div>
                  <div className={styles.actions}>
                    <button disabled={busy || provider.trim().length < 2} onClick={() => void action("shipment_ready")}>Salva come ready</button>
                    <button className={styles.primary} disabled={busy || provider.trim().length < 2 || trackingCode.trim().length < 3} onClick={() => void action("shipped")}>Segna shipped</button>
                  </div>
                </section>
              ) : null}

              {detail.order.status === "shipped" ? (
                <div className={styles.actions}><button className={styles.primary} disabled={busy} onClick={() => void action("delivered")}>Segna delivered</button></div>
              ) : null}

              <section className={styles.notifications}>
                <div><p className={styles.kicker}>CUSTOMER EMAIL OUTBOX</p><h2>Notifiche</h2></div>
                {detail.notifications.length ? detail.notifications.map((notification) => (
                  <article key={notification.id}><strong>{notification.kind}</strong><span>{notification.status}</span><small>{notification.attempts} attempts · {date(notification.sentAt ?? notification.updatedAt)}</small>{notification.lastErrorCode ? <em>{notification.lastErrorCode}</em> : null}</article>
                )) : <p className={styles.empty}>Nessuna notifica ancora accodata.</p>}
                <p className={styles.note}>La coda è provider-neutral: nessuna email esterna viene inviata finché non colleghiamo e abilitiamo un provider.</p>
              </section>
            </>
