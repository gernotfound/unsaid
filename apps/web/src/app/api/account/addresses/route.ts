import { createCustomerAddress } from "@unsaid/db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const address = await createCustomerAddress(session.uid, addressInput(body));
    return Response.json({ ok: true, address }, { status: 201 });
  } catch (error) {
    if (error instanceof CustomerAuthError) return apiError(error.code, 401);
    if (error instanceof Error && error.message.startsWith("INVALID_ADDRESS:")) {
      return apiError(error.message);
    }
    if (error instanceof Error && error.message === "ADDRESS_LIMIT_REACHED") {
      return apiError("ADDRESS_LIMIT_REACHED", 409);
    }
    logError("customer.address.create_failed", error, { requestId, route: "/api/account/addresses" });
    return apiError("ADDRESS_CREATE_FAILED", 500);
  }
}
